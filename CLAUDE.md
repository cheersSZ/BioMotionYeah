...
- **Per-foot ground tracking**: `NimbleBridge` tracks `_groundHeightLY` and `_groundHeightRY` independently (each foot ratchets to its own `calcn` minimum). Contact detection uses hysteresis (`CONTACT_ATTACH = 0.06 m` to attach, `CONTACT_RELEASE = 0.08 m` to release). The legacy `groundHeightY` accessor returns `min(L, R)` for UI compatibility. Single-bar logic is what historically produced bilateral muscle-activation asymmetry (e.g. tibialis anterior) on otherwise symmetric input.
- **Offline `noGRF` mode**: `NimbleEngine.idMode = .noGRF` skips GRF estimation entirely (calls `bridge.solveID` instead of `solveIDGRF`). Used as the structural-symmetry control for asymmetry diagnosis. Mode is surfaced as a toggle in `OfflineAnalysisView` and tagged into the `<stem>_summary.json` as `idMode: "withGRF" | "noGRF"`.
